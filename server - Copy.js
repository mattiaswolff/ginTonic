...
//Mongoose
mongoDB = mongoose.createConnection(config.mongodb_connectionString, { server:{
        auto_reconnect: true,
        poolSize: 10,
        socketOptions:{
            keepAlive: 1
...
applications_model = mongoDB.model('applications', applicationSchema);
...
app.get('/api/applications/:application', function (req, res) {
  ...
  logHandler('INFO', 'Express', logObject);
  applications_model.findOne(
    {name: req.params.application.toLowerCase()},
    function (err, results) {
      if (!err) {
        if (results) {
          res.json(results);  
...
app.get('/api/containers/:container/files/:file', function (req, res) {
  ...
  if (containerName === "applications") {
    applications_model.findOne(
      { name: blobName.split("_")[0] }, 
      function (err, doc) {
        if (!err) {
          doc.meta.downloadCount = doc.meta.downloadCount + 1;
          doc.save();  
        } else {
          logHandler('ERROR', 'MongoDB', err);
        }
...
app.get('/api/applications/:application/versions/:version', function (req, res) {
  ...
  applications_model.findOne(
    ...
    function (err, results) {
      if (!err) {
        if (results) {
          i = results.versions.length;
          strRegexp = req.params.version.replace(/\./g , "\\.").replace(/\*/g , ".+");
  ...
  logHandler('INFO', 'Express', logObject);
    async.series([
      ...
      function(callback) {
        ...
        parser.parseString(manifestRaw, function (err, result) {
          if (!err)  {
              if (result) {
                manifest = result;
      ...
      function(callback) {
        applications_model.findOne({ name : manifest.Package.$["Name"].toLowerCase() }, function (err, res) {
          if (!err) {
            if (!res) {
            applications_model.create({
              name : manifest.Package.$["Name"].toLowerCase(),
              friendlyName : manifest.Package.$["Name"].toLowerCase(),
              description : '',
              category : '',
            }, function(err, res) {
                if (!err) {
      ...
      function(callback) {
        if (app) {
          ...
          if (manifest.Package["Dependecies"][0]["Package"]) {
          ...
          version.downloadUrl = config.serverUrlBase + "/api/containers/applications/files/" + app.name.toLowerCase() + "_" +  version.version.toLowerCase() + ".zip";
          ...
          if (req.body.friendlyName) {
            app.friendlyName = req.body.friendlyName;  
          }
          ...
          err = 'Add new version to registry failed';
          logHandler('ERROR', 'Nodejs', err);
      ...
      function(callback) {
        var blobName = app.name.toLowerCase() + "_" +  version.version.toLowerCase() + ".zip";
        
        createBlockBlobFromFileModForLargeFiles (
          req.files.applicationPackage.path, 
          blobName, 
          "applications",
          function(err){
              if(!err){
...
/*
        blobService.createBlockBlobFromFile("applications"
          ...
          , function(err){
              if(!err){
                logHandler('INFO', 'BlobStorage', 'Blob uploaded: ' + blobName);
                fs.unlink(req.files.app.path, function (err) {
                  if (err) throw err;
                  logHandler('INFO', 'BlobStorage', 'Temp file deleted: ' + req.files.app.path);
                  callback();
                });                
              }
              else {
...
function createBlockBlobFromFileModForLargeFiles (file, blobName, container, callback) {
  ...
  async.timesSeries(chunks, function (n, next) {
    ...
    blobService.createBlobBlockFromStream(blockId, container, blobName, stream, end-start, function(error){
      logHandler('INFO', 'BlobStorage', 'Chunk done: ' + n + ', ' + error);
